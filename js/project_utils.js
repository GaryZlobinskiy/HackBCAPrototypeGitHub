...
function confirmDelete(project_name, project_id, redirect_to) {
  if (
    confirm(
      `Are you sure you want to delete the project ${project_name} (id ${project_id})? This is PERMANENT and cannot be undone.`
    )
  ) {
    //TODO: make delete request
    alert(
      `Successfully deleted the project ${project_name} (id ${project_id}).`
    );
